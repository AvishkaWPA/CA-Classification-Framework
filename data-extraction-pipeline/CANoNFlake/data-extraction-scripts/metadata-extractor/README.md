...
2. **Active Bugs:** Reads the `active-bugs.csv` file for each project to get:
   - The bug ID.
   - The Git SHA of the buggy commit (`revision.id.buggy` mapped to `issue_commit`).
...
5. **CSV/JSONL Creation:** Pre-populates the 11-column schema with `issue_category` set to `"Non-Flaky"`, the project's GitHub repository URL from `config.py`, and empty values for later extraction phases.
