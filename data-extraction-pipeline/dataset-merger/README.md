...
*   **Aligns & Concatenates:** Reads both `CAFlake/context_enriched_dataset.csv` and `CANonFlake/non_flaky_dataset.csv` (since they share the identical 12-column schema) and concatenates them.
*   **Keeps Binary Labels:** Utilizes the `isFlaky` column:
    -   `1` represents flaky test failure samples.
    -   `0` represents deterministic, non-flaky test failure samples.
...
*   `data-extraction-pipeline/context_augmented_dataset.csv`
*   `data-extraction-pipeline/context_augmented_dataset.jsonl` (if `.jsonl` sources are present)
